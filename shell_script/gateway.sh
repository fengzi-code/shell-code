...
fun_mvn () {
echo $local_lujin
cd $local_lujin  && git checkout . && git clean -xdf && git pull $git_url && mvn package -Dmaven.test.skip=true && cd $lujin
echo "完毕"
}
...
fun_script () {
        ...
        project_name=$(echo $fuwu_name |sed "s/eagle-//g")
        DIR_SRC=git/eagle-services/eagle-gateway
        DIR_DST=/home/eagle/eagle-gateway
        ...
        cat > ${DIR_SRC}/${fuwu_name}/target/${startsh} << EOF
#!/bin/bash
source /etc/profile
nohup java -jar -Dcom.sun.management.jmxremote -Dcom.sun.management.jmxremote.port=1$fuwu_port -Dcom.sun.management.jmxremote.ssl=false -Dcom.sun.management.jmxremote.authenticate=true -Djava.rmi.server.hostname=$ipdz -Dcom.sun.management.jmxremote.password.file=$JAVA_HOME/jmxremote.password -Dcom.sun.management.jmxremote.access.file=$JAVA_HOME/jmxremote.access -Xms1024m -Xmx1024m ${fuwu_name}-0.1.jar --profiles=prod --defaultZone="http://eureka01.eagle.mzj.net:1110/eureka,http://eureka02.eagle.mzj.net:1110/eureka" --logging.level.root=WARN --port=${fuwu_port} --logging.file=/home/eagle/eagle-gateway/${project_name}/log/${fuwu_name}.log >/dev/null &
echo "启动完毕"
exit
EOF
        ...
        cat > ${DIR_SRC}/${fuwu_name}/target/${stopsh} << EOF
#!/bin/bash
...
EOF
        ...
        #清理文档目录
        cat > ${DIR_SRC}/${fuwu_name}/target/clear.sh << EOF
#!/bin/bash
find /home/eagle/eagle-gateway/bak/  -type d -name "*_bak" -mtime +7 -exec rm -rf {} \; || true
mkdir -p /home/eagle/eagle-gateway/bak/${project_name}_bak
mkdir -p $JAVA_HOME
cp -r /home/eagle/eagle-gateway/${project_name}   /home/eagle/eagle-gateway/bak/${project_name}_bak/${project_name}_`date +%Y%m%d_%H%M%S`_bak || true
rm -rf  /home/eagle/eagle-gateway/${project_name}/lib/*.jar || true
mkdir -p log
...
fun_deploy () {
        ...
        scp -r ${DIR_SRC}/${fuwu_name}/target/clear.sh $ipdz:${DIR_DST}/${project_name}/
        ssh ${ipdz} "chmod 755 ${DIR_DST}/${project_name}/*.sh;cd ${DIR_DST}/${project_name} && sh clear.sh &"
        scp -r ${DIR_SRC}/${fuwu_name}/target/lib ${DIR_SRC}/${fuwu_name}/target/*.sh ${DIR_SRC}/${fuwu_name}/target/*.jar $ipdz:${DIR_DST}/${project_name} 
...
fuwu="
序号  ---  -----微服务------------------  -端口-----#
序号  1.   EAGLE-CONFIG-SERVER           1130
序号  2.   EAGLE-EUREKA                  1110
序号  3.   EAGLE-ZIPKIN                  1140
序号  4.   EAGLE-ZUUL                    1120
"
}
...
git@gl.mzjmedia.net:gateway/eagle-gateway.git config-server.git eureka-server.git zipkin-server.git eagle-zuul.git
"
#git_url=$(echo -e "${git_url_all}" | grep $fuwu_name.git |awk '{print $1}')
git_url=git@gl.mzjmedia.net:gateway/eagle-gateway.git
local_lujin=/apps/deploy/git/eagle-services/eagle-gateway
}
